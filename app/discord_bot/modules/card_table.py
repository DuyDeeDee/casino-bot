...
def compose_card_table(
    dealer_hand: list[Image.Image] | None,
    ...
    *,
    ...
    base_w, base_h = dealer_hand[0].size if dealer_hand else player_hands[0][0].size
    col_gap = 10
    padding = 16
    ...
    max_cards_dealer = len(dealer_hand) if dealer_hand else 0
    max_cards_player = max(len(hand) for hand in player_hands)
    rows_per_lane = max(len(lane) for lane in player_lanes)

    dealer_width_limit = (available_w - ((max_cards_dealer - 1) * col_gap)) / max_cards_dealer if max_cards_dealer else available_w
    player_width_limit = (lane_width - ((max_cards_player - 1) * col_gap)) / max_cards_player

    available_h = max(1, bg_h - (padding * 2))
    vertical_card_limit = (
        available_h
        - (dealer_to_player_gap if max_cards_dealer else 0)
        - ((rows_per_lane - 1) * player_row_gap)
    ) / (rows_per_lane + (1 if max_cards_dealer else 0))
    ...
    total_h = (
        (card_h if dealer_hand else 0)
        + (dealer_to_player_gap if dealer_hand else 0)
...
def render_card_table(
    ...
    dealer_hand: Sequence[Card] | None,
    ...
    *,
    ...
    dealer_images = hand_to_images(dealer_hand) if dealer_hand else []
