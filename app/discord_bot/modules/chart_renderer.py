...
def draw_stock_chart(symbol: str, history: list[int], current_price: int) -> io.BytesIO:
    # Một điểm dữ liệu duy nhất không vẽ được đường -> nhân đôi để ra đường ngang
    if len(history) == 1:
    ...
    # Chỉ đánh số mỗi bước label_step để chữ không chồng nhau khi nhiều điểm
    label_step = max(1, num_points // 6)
    points = []
