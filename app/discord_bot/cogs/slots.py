...
class Slots(commands.Cog):
    ...
    @commands.command(
        brief="Máy quay xèng (Slots)\nMức cược phải từ 1-3 thỏi vàng",
        usage="slots [mức_cược]",
    )
    async def slots(self, ctx: commands.Context, bet: int = 1):
        ...
        status_text = "thua" if result[0] == "lost" else "thắng"
        punctuation = "." if result[0] == "lost" else "!"
        embed = make_embed(
            title=f"Bạn đã {status_text} {result[1]} thỏi vàng{punctuation}",
            description=(
                "Hiện tại bạn có "
