...
class HighCardRedrawView(discord.ui.View):
        ...
        if not interaction.user or interaction.user.id != self.user_id:
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    "Chỉ người chơi bắt đầu lượt này mới có thể sử dụng nút.",
                    ephemeral=True,
                )
            else:
    ...
    @discord.ui.button(label="Bốc lại mức cược này", style=discord.ButtonStyle.primary)
    async def redraw(self, interaction: discord.Interaction, _: discord.ui.Button):
        if self._redraw_in_progress:
            await interaction.response.send_message(
                "Đang tiến hành bốc lại bài.",
                ephemeral=True,
            )
            return
...
class Gambling(commands.Cog):
        ...
        *,
        ...
        try:
            embed = make_embed(
                title=title,
                description=(
                    f"Nhà cái: **{dealer_card}**\n"
                    f"Người chơi: **{player_card}**\n"
                    f"Mức cược: **{bet:,} VND**\n"
                    f"Lợi nhuận ròng: **{self._format_delta(delta)}**\n"
                    f"Số dư: **{balance:,} VND**"
                ),
    ...
    @commands.command(
        brief="Chơi tung đồng xu\nMức cược phải lớn hơn 0 VND",
        usage=f"flip <heads|tails> [tiền_cược - mặc định={config.bot.default_bet:,} VND]",
    )
    async def flip(
        self,
        ctx: commands.Context,
    ...
    @commands.command(
        brief="Đổ xúc xắc 6 mặt\nMức cược phải lớn hơn 0 VND",
        usage=f"roll <dự_đoán:1-6> [tiền_cược - mặc định={config.bot.default_bet:,} VND]",
    )
    async def roll(
        self,
        ctx: commands.Context,
    ...
    @commands.command(
        brief="Rút bài so điểm với nhà cái.\nMức cược phải lớn hơn 0 VND",
        usage=f"highcard [tiền_cược - mặc định={config.bot.default_bet:,} VND]",
        aliases=["war"],
    )
    async def highcard(
        self,
