...
# Read using utf-8 with errors='replace' to ignore/replace the bad byte at line 1819
with open(daga_path, "r", encoding="utf-8", errors="replace") as f:
    content = f.read()

# Clean up duplicate/corrupted block first
train_trigger = "await self._trigger_random_event(ctx, cock)"
first_train = content.find(train_trigger)
correct_fight = "Thách đấu đá gà PvP đặt cược với người chơi khác."
fight_text = content.find(correct_fight)
if first_train != -1 and fight_text != -1:
    end_of_train = first_train + len(train_trigger)
    decorator_idx = content.rfind("@daga_group.command", 0, fight_text)
    if decorator_idx != -1:
        line_start = content.rfind("\n", 0, decorator_idx)
        if line_start != -1:
            decorator_idx = line_start + 1
        content = content[:end_of_train] + "\n\n" + content[decorator_idx:]
        print("Successfully cleaned up the duplicated/corrupted block!")

# Add CHARACTER_INFO_MAP before RARITY_DISPLAY
if "CHARACTER_INFO_MAP" not in content:
    rarity_display_idx = content.find("RARITY_DISPLAY = {")
...
"""
    content = content[:rarity_display_idx] + map_code + content[rarity_display_idx:]

# ----------------- FIX 1: CHARACTER SKILLS / BATTLE LOOP -----------------
...
new_battle_loop = """        # Initialize combat states
        combat_state = {
            c1.id: {
                "hp": hp1,
                "max_hp": hp1,
...
# Reconstruct battle loop
if start_match:
    content = content.replace(start_match.group(0), new_battle_loop)
else:
    start_match2 = re.search(r'        # Initialize combat states.*?(?=\s+# Sync back final HPs for the checks below)', content, re.DOTALL)
    if start_match2:
        content = content.replace(start_match2.group(0), new_battle_loop)
    else:
        print("Error: Could not locate block regex matching combat loop!")
        exit(1)

# ----------------- FIX 2: CHARACTER INFO CARD REDESIGN -----------------
info_start = content.find("    async def daga_info(self, ctx: commands.Context, cock_id: int | None = None):")
if info_start == -1:
...
target_start_idx = content.find(target_start_str, info_start)
target_end_idx = content.find(target_end_str, target_start_idx)
...
"""
content = content[:target_start_idx] + new_daga_info_body + content[target_end_idx:]

# ----------------- FIX 3: REPLACE CORRUPTED DAGA_EQUIP -----------------
# Let's locate the corrupted daga_equip block
equip_start = content.find("    @daga_group.command(name=\"equip\"")
list_start = content.find("    @daga_group.command(name=\"list\"")

if equip_start == -1 or list_start == -1:
    print("Error: Could not locate equip/list markers!")
    exit(1)

clean_equip_code = """    @daga_group.command(name="equip", brief="Lắp trang bị từ kho đồ cho nhân vật chính bằng ID (1-18).")
    async def daga_equip(self, ctx: commands.Context, equip_id: str):
        equip_id = equip_id.strip()
        if equip_id not in GEAR_ID_MAP:
            await ctx.send("❌ **Lỗi:** ID trang bị không hợp lệ! Vui lòng chọn từ `1` đến `18`:\\n"
                           "Gõ `i?anime shop` để xem ID tương ứng.")
            return
            
        item_id, slot, item_fullname = GEAR_ID_MAP[equip_id]
        
        active_row = self.economy.get_active_cock(ctx.author.id)
        if not active_row:
            await ctx.send("❌ **Lỗi:** Bạn chưa chọn nhân vật chính xuất trận nào. Hãy dùng `i?anime active`.")
            return
        ...
        if item_qty <= 0:
            await ctx.send(f"❌ **Lỗi:** Bạn không sở hữu trang bị {item_fullname} trong kho đồ.")
            return

        item_reverse_mapping = {
            "Cựa Gỗ": ("weapon", "cua_go"),
            "Giáp Da": ("armor", "giap_da"),
            "Khăn Đỏ": ("charm", "khan_do"),
            "Cựa Sắt": ("weapon", "cua_sat"),
            "Giáp Đồng": ("armor", "giap_dong"),
            "Chuông Bạc": ("charm", "chuong_bac"),
            "Cựa Thép": ("weapon", "cua_thep"),
            "Giáp Thép": ("armor", "giap_thep"),
            "Bùa Ngọc": ("charm", "bua_ngoc"),
            "Cựa Dao": ("weapon", "cua_dao"),
            "Giáp Hổ": ("armor", "giap_ho"),
            "Dây Chuyền Phượng": ("charm", "day_chuyen_phuong"),
            "Cựa Thiên Lôi": ("weapon", "cua_thien_loi"),
            "Giáp Kim Cang": ("armor", "giap_kim_cang"),
            "Ngọc Long": ("charm", "ngoc_long"),
            "Cựa Diệt Thần": ("weapon", "cua_diet_than"),
            "Giáp Thần Thú": ("armor", "giap_than_thu"),
            "Linh Châu": ("charm", "linh_chau"),
        }

        current_val = getattr(cock, slot)
        unequipped_msg = ""
        if current_val and current_val != "None":
            old_slot, old_item_id = item_reverse_mapping[current_val]
            self.economy.add_inventory_item(ctx.author.id, old_item_id, 1)
            unequipped_msg = f"Đã tháo **{current_val}** cất vào kho đồ. "

        self.economy.add_inventory_item(ctx.author.id, item_id, -1)
        self.economy.update_cock(cock.id, **{slot: item_fullname})

        await ctx.send(f"✅ {unequipped_msg}Đã lắp thành công trang bị **{item_fullname}** cho nhân vật **{cock.name}** (ID: {cock.id})!")
"""

# Replace the corrupted block
content = content[:equip_start] + clean_equip_code + content[list_start:]

# Save
with open(daga_path, "w", encoding="utf-8") as f:
    f.write(content)

print("Successfully applied all fixes (skills, redesigned info card, clean equip command)!")
